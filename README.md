...
Stream2Grid 是一个专为 Obsidian 用户设计的 Chrome 浏览器扩展。它能够让你在浏览网页时，通过简单的“拖拽”或“采集”流，快速收集文本、图片和链接，并最终一键导出为 Obsidian Canvas (.canvas) 格式的网格布局。

## 🚀 核心特性

- **极简采集流**：在浏览器侧边栏以垂直列表形式管理你的灵感。
- **混合拖拽**：支持从网页直接拖入，也支持列表内自由排序。
- **一维转二维**：导出时自动将列表转化为 Obsidian 中的网格布局。
