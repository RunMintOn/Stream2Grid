...
import { saveImage, verifyPermission } from '../../services/fs';
...
export default function LocalMDView({ fileHandle, project: _project }: LocalMDViewProps) {
  ...
  // Load file content on mount or handle change
  useEffect(() => {
    loadContent();
  }, [loadContent]);

  // ... (rest of the file remains similar but handles permission)
       ...
       try {
         if (detail.type === 'text') {
           ...
           if (vault && vault.fileHandle) {
              ...
              // We might need a custom file handling logic in BlockNote or just use Blob URL for display?
    ...
    return (
      <div className="h-full w-full flex items-center justify-center bg-slate-50 p-4">
        <div className="max-w-xs text-center">
          ...
          <p className="text-slate-800 font-medium mb-2">需要访问权限</p>
          <p className="text-slate-500 text-xs mb-6 leading-relaxed">
            浏览器安全限制要求每次重新启动或切换项目时可能需要重新确认对本地文件的访问权限。
          </p>
          <button 
            onClick={loadContent} 
            className="w-full py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors shadow-sm font-medium"
          >
            点击授权访问
          </button>
        </div>
