  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-4 group relative overflow-visible">
      <div className="absolute inset-0 z-40 pointer-events-none">
        <a href={url} target="_blank" rel="noopener noreferrer" className="absolute top-0 left-0 w-[42%] h-[42%] pointer-events-auto group/tl" aria-label="Open Link" />
        <a href={url} target="_blank" rel="noopener noreferrer" className="absolute top-0 right-0 w-[42%] h-[42%] pointer-events-auto group/tr" aria-label="Open Link" />
        <a href={url} target="_blank" rel="noopener noreferrer" className="absolute bottom-0 left-0 w-[42%] h-[42%] pointer-events-auto group/bl" aria-label="Open Link" />
        <a href={url} target="_blank" rel="noopener noreferrer" className="absolute bottom-0 right-0 w-[42%] h-[42%] pointer-events-auto group/br" aria-label="Open Link" />
      </div>
      ...
      <button
        onClick={handleDelete}
        className="absolute bottom-0 right-0 translate-x-1/4 translate-y-1/4 w-7 h-7 flex items-center justify-center rounded-full shadow-md transition-all duration-200 opacity-0 group-hover/br:opacity-100 hover:scale-110 active:scale-95 z-30 bg-[#e05f65] hover:bg-[#af3029] text-[#fffcf0] pointer-events-auto"
        aria-label="Delete"
      >
